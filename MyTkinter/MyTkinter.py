...
class ImageViewer(tk.Label):
    def __init__(self, master, img: str, img_height: int, *args, **kwargs):
        super(ImageViewer, self).__init__(master, *args, **kwargs)
        self.img = None
        self.set_img(img, img_height)

    def set_img(self, filename, img_height):
        self.img = cv2.imread(filename)
        self.img = photo_from_ndarray(self.img, img_height)
        setLabelImg(self, self.img)


class PlateDetectCam(ImageViewer):
    def __init__(self, network=ImageProcess.PlateDetect.TINY_MODEL,
                 min_confidence: float = 0.5,
                 *args, **kwargs):
        self.network = network
        self.min_confidence = min_confidence
        super(PlateDetectCam, self).__init__(*args, **kwargs)

    def set_img(self, filename, img_height):
        self.img = cv2.imread(filename)
        ImageProcess.PlateDetect.detectPlate(self.img, network=self.network, min_confidence=self.min_confidence)
        self.img = photo_from_ndarray(self.img, img_height)
        setLabelImg(self, self.img)
